...
import numpy as np
from layers.capsule import LinearCaps2d, CapsPrimary2d, FCDecoder, FCDecoder2
from ops.utils import conv2d_output_shape


class GammaCapsNet(nn.Module):
    ...
    def __init__(self, config, device, bins=None):
        ...
        self.fcCaps = LinearCaps2d(input_height=h1,
                                     input_width=w1,
                                     routing_method="sda",
                                     ...
                                     output_caps_types=32,
                                     output_caps_shape=[8,1],
        ...
        self.classCaps = LinearCaps2d(input_height=1,
                                     input_width=1,
                                     routing_method="sda",
                                     num_iterations=config.num_routing_iterations,
                                     input_caps_types=32,
                                     input_caps_shape=[8,1],
        ...
        if config.reconstruction == "FCDecoder":
            ...
            self.decoder = FCDecoder2(config,
                                      in_features_fc1=config.dim_classCaps[0] * config.dim_classCaps[1],
                                      out_features_fc1=config.out_features_fc1,
                                      out_features_fc2=config.out_features_fc2,
                                      out_features_fc3=config.out_features_fc3,
                                      device=device)
    ...
    def forward(self, x, target=None):
        ...
        if self.config.coupl_coeff:
            output_caps_poses, output_caps_activations, coupling_coefficients = self.fcCaps(output_caps_poses, output_caps_activations, True)
            ...
            output_caps_poses, output_caps_activations = self.fcCaps(output_caps_poses, output_caps_activations)
