...
class vectorConvCapsNet(nn.Module):

    def __init__(self, config, device, bins=None):
        super(vectorConvCapsNet, self).__init__()
        self.config = config
        self.device = device
        self.bins = bins
        ...
        self.conv2 = nn.Conv2d(in_channels=config.out_channels_conv1,
                               out_channels=config.out_channels_conv1,
                               kernel_size=config.kernel_size_conv1,
                               stride=config.stride_conv1)

        self.h0, self.w0 = conv2d_output_shape((self.h0, self.w0),
                                     kernel_size=config.kernel_size_conv1,
                                     stride=config.stride_conv1,
                                     pad=config.padding_conv1,
                                     dilation=config.dilation_conv1)

        self.conv3 = nn.Conv2d(in_channels=config.out_channels_conv1,
                               out_channels=config.out_channels_conv1,
                               kernel_size=config.kernel_size_conv1,
                               stride=config.stride_conv1)
